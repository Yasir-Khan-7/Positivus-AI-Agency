  ...
  return (
    <section className="w-full bg-white">
      <div className="max-w-[1440px] mx-auto px-8 lg:px-[100px] py-[70px]">
        ...
        <div className="bg-dark rounded-[45px] p-[60px] lg:p-[70px]">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-[40px] lg:gap-[50px] divide-y lg:divide-y-0 lg:divide-x divide-white/20">
            {caseStudies.map((study, index) => (
              <div
                key={study.id}
                className={`${index > 0 ? 'pt-[40px] lg:pt-0 lg:pl-[50px]' : ''}`}
              >
                <p className="text-[18px] font-normal leading-[23px] text-white mb-[20px]">
                  {study.description}
                </p>
                <Link
                  href="#"
                  className="inline-flex items-center gap-[15px] text-primary hover:opacity-70 transition-opacity"
                >
